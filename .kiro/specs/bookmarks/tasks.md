...
- [x] `config/initializers/rack_attack.rb` を作成する
- [x] `/api/` パスへのリクエストに対してIPベースのスロットリングを設定する
- [x] 制限値を環境変数 `RATE_LIMIT_PER_MINUTE` から取得する（デフォルト10）
- [x] 制限期間を1分に設定する
...
- [x] 超過時に429ステータスコードを返す設定を行う
- [x] JSON形式のエラーレスポンスを返す
- [x] エラーコード `rate_limit_exceeded` とメッセージを含める
...
- [x] `GET /health` エンドポイントを実装する
- [x] ステータス200と `{"status":"ok"}` を返す
- [x] 認証をスキップする設定を行う
- [x] レートリミットの対象外にする
